...
const onClickButtonReset = (event) =>{
    ...
    root.querySelectorAll('.right-box.triangle .drag-zone').forEach((zone, index)=>{
    ...
    root.querySelectorAll('.right-box.parallelogram .drag-zone').forEach((zone, index)=>{
        setTransform(zone, 0, 0, 0);
        if(index ===0)
            zone.style.display = 'block';
        else
...
const checkTrianglePosition = (dragObject, dropObject) => {
    // 삽질했는데, 삼각형이 정 삼각형이 아니다. 그냥 편법으로 처리한다.
    const recDrag = dragObject.getBoundingClientRect();
    const recDrop = dropObject.getBoundingClientRect();
    const dropCenterX = (recDrop.left + recDrop.right)/2;
    const dropCenterY = (recDrop.top + recDrop.bottom)/2;
    const parent = dragObject.parentElement.parentElement;
    const parentTransform = getTransform(parent);
    const dropDegree = dropObject.classList.contains('reverse')?180:0;
    const degreeDiff = (dropDegree - parentTransform[2] + 360) %360;
    let degreeOK = false;
    if(degreeDiff < 10 || degreeDiff > 350
        || (degreeDiff > 110 && degreeDiff < 130)
        || (degreeDiff > 230 && degreeDiff < 250)){
        degreeOK = true;
    }
    let distanceOK = false;
    if((Math.abs(recDrag.left - recDrop.left) < 20 || Math.abs(recDrag.right - recDrop.right) < 20)
        && (Math.abs(recDrag.top - recDrop.top) < 20 || Math.abs(recDrag.bottom - recDrop.bottom) < 20)){
        distanceOK = true;
    }
    if(degreeOK && distanceOK){
        // 먼저 회전 시킨다.
        setTransform(parent, parentTransform[0], parentTransform[1], dropDegree);
        const recNew = dragObject.getBoundingClientRect();
        const centerX = (recNew.left + recNew.right)/2;
        const centerY = (recNew.top + recNew.bottom)/2;
        const x = (dropCenterX - centerX) / (scale / 100) + parentTransform[0];
        const y = (dropCenterY - centerY) / (scale / 100) + parentTransform[1];
        return [x, y, dropDegree];
    }
    return null;
}
const checkParallelogramPosition = (dragObject, dropObject) => {
    ...
    const degreeDiff = (dropDegree - parentTransform[2] + 360) %360;
    let degreeOK = false;
    if(degreeDiff < 10 || degreeDiff > 350 || (degreeDiff > 170 && degreeDiff < 190)){
        degreeOK = true;
    }
    let distanceOK = Math.abs(xDiff) < 15 && Math.abs(yDiff) < 15;
    if(degreeOK && distanceOK) {
...
const checkDragPosition = (dragObject, rightBoxClass) => {
    const parent = dragObject.parentElement.parentElement;
    const dropZones = root.querySelectorAll(`.left-box.${rightBoxClass} path.drop`)
    for(let i = 0; i < dropZones.length; i++){
        const drop = dropZones[i];
        if(drop.dragObject)
            continue;
        const checkResult = rightBoxClass==="triangle"?checkTrianglePosition(dragObject, drop)
            : checkParallelogramPosition(dragObject, drop);
        if(checkResult){
            drop.dragObject = dragObject;
            drop.classList.add('complete');
            setTransform(parent,  checkResult[0],  checkResult[1], checkResult[2]);
            parent.style.opacity = 0;
            // drag 중에 display가 none 인 것을 찾아서 하나 활성한다.
            let dragZones = root.querySelectorAll(`.right-box.${rightBoxClass} .drag-zone`);
    ...
    // 여기까지 왔으면 원래 위치로 돌린다. 먼저 오른쪽에 있는 도형은 지운다.
    root.querySelectorAll(`.right-box.${rightBoxClass} .drag-zone`).forEach((zone)=>{
...
const setDragAndDrop = () => {
    root.querySelectorAll(".right-box").forEach((rightBox)=>{
        const rightBoxClass = rightBox.classList.contains('triangle')?"triangle":"parallelogram"
        const dragZone = rightBox.querySelector(".drag-zone");
        const dragCount = rightBoxClass==="triangle"? 3: 1;
        ...
        rightBox.querySelectorAll("svg > path").forEach((polygon) => {
            const _onMouseDown = (event) => {
                ...
                root.querySelectorAll(`.left-box.${rightBoxClass} path.drop`).forEach((drop) => {
                ...
                const onMouseMove = (event) => {
                    ...
                    const filterResult = underElements.filter((value) =>
                        value.classList.contains('drop') && value.classList.contains(rightBoxClass)
                    );
                    ...
                    if(draggedIn && filterResult.length < 1){
                        ...
                        dragObject = null;
                        removeDragListener();
                        rightBox.querySelectorAll(".drag-zone").forEach((zone)=>{
                            if(zone.style.display !== 'none' && zone.style.opacity!=="0" && zone !== parent)
                                zone.style.display = 'none';
                        });
                        setTransform(parent, 0, 0, 0);
                ...
                const onDragEnd = (event, isDocumentLeave = false) => {
                    removeDragListener();
                    // 모든 회전 버튼을 안보이게 한다.
                    root.querySelectorAll('.right-box .btn-rotate').forEach((button)=>button.style.display = 'none');
                    // 이 버튼만 보이게 한다.
                    ...
                    if(polygon === dragObject) {
                        ...
                        const y = (clientY - parent.dataset.y) / (scale / 100) + 1 * parent.dataset.translateY;
                        setTransform(parent, x, y, parent.dataset.rotateAngle);
                        checkDragPosition(polygon, rightBoxClass);
                    }
                }
...
const setRotate = () => {
    ...
    root.querySelectorAll('.right-box').forEach((box) => {
        const boxClass = box.classList.contains('triangle')?"triangle":"parallelogram"
