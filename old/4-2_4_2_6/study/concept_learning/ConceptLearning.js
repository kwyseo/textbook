...
export default class ConceptLearning{
    ...
    constructor(root) {
        ...
        this.polygon = [
            ['triangle', '삼각형', 'triangle_small.png'],
            ['parallelogram', '사각형', 'parallelogram.png'],
            ['rhombus', '사각형', 'rhombus.png'],
            ['hexagon', '육각형', 'hexagon_small.png'],
            ['trapezoid', '사각형', 'trapezoid.png'],
            ['square', '사각형', 'square.png']
        ];
        this.regularPolygon = [
            ['triangle', '정삼각형', 'triangle_small.png'],
            ['hexagon', '정육각형', 'hexagon_small.png'],
            ['square', '정사각형', 'square.png']
        ];
    ...
    getRandomNumberExcluding(min, max){
        let randomNumber;
        do { randomNumber = Math.floor(Math.random() * (max - min + 1)) + min; }
        while (this.selectedIndex===randomNumber);
        this.selectedIndex = randomNumber;
        return randomNumber;
    }
    ...
    activateQuestion(containerIndex){
        ...
        */
        ...
        // 도형을 골라야 한다.
        if(containerIndex===0){
            const targetIndex = this.getRandomNumberExcluding(0, this.polygon.length -1);
            this.root.querySelectorAll('.question_polygon').forEach((polygon)=>{
                this.polygon.forEach((element)=>{
                    polygon.classList.remove(element[0]);
                });
                polygon.classList.add(this.polygon[targetIndex][0]);
                polygon.setAttribute('src', '../study/img/' + this.polygon[targetIndex][2]);
            });
            // 정답도 바꿔야지...
            this.root.querySelector('.polygon_answer').innerHTML = this.polygon[targetIndex][1];
        }else{
            const targetIndex = this.getRandomNumberExcluding(0, this.regularPolygon.length -1);
            this.root.querySelectorAll('.question_regular_polygon').forEach((regularPolygon)=>{
                this.regularPolygon.forEach((element)=>{
                    regularPolygon.classList.remove(element[0]);
                });
                regularPolygon.classList.add(this.regularPolygon[targetIndex][0]);
                regularPolygon.setAttribute('src', '../study/img/' + this.regularPolygon[targetIndex][2]);
            });
            // 정답도 바꿔야지...
            this.root.querySelector('.regular_polygon_answer').innerHTML = this.regularPolygon[targetIndex][1];
        }
    }
