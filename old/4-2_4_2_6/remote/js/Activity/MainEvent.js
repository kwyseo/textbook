...
export class MainEvent {
    ...
    closePopup(){
        ...
        document.querySelectorAll("#popup > div").forEach((div)=>{
    ...
    Init() {
        ...
        this.problem_solving.addEventListener("callToParent", (event) => {
            {
                ...
                if (event.detail.message.type === "popup") {
                    this.popup.classList.remove("hide");
                    const container = event.detail.message.container;
                    const target = event.detail.message.target;
                    if(container===0){
                        this.popup.querySelectorAll('.dim').forEach((dim, dimIndex)=>{
                            if(dimIndex === target)
                        ...
                        this.popup.querySelectorAll('.container2').forEach((dim, dimIndex)=>{
                            if(dimIndex === target)
                                dim.classList.remove('hide');
                            else
                                dim.classList.add('hide');
                    ...
                    if (this.checkPhone()) {
                        ...
                        if (index === 0) {
                            this.keyboard.style.top = "320px";
                            tail.style.top = "355px";
                        } else if (index === 1) {
                            this.keyboard.style.top = "320px";
                            tail.style.top = "425px";
                        }else{
                            this.keyboard.style.top = "320px";
                            tail.style.top = "490px";
                        }
                        this.keyboard.classList.remove('hide');
                        this.keyboardTarget = target;
                    }
        ...
        this.popup.querySelectorAll('.button_yellow_o').forEach((close) => {
            close.addEventListener("click", (e) => {
                this.closePopup();
                this.problem_solving.querySelector("remote-html-embed").shadowRoot.clickOK();
            })
        });
        this.popup.querySelectorAll('.button_yellow_x').forEach((close) => {
            close.addEventListener("click", (e) => {
                this.closePopup();
                this.problem_solving.querySelector("remote-html-embed").shadowRoot.clear();
            })
        });
        this.capture.addEventListener("click", (e) => {
            let fileName = '4학년_2학기_4단원_2~6차시_';
