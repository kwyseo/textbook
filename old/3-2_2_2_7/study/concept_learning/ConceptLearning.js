    ...
    constructor(root) {
        ...
        if(root.querySelector(".check_btn")) {
            root.querySelector(".check_btn").onclick = (event) => {
                ...
                // 묶음을 다 만들었는지 확인한다.
                let isComplete =true;
                const numerator = this.root.querySelector('.numerator_value').innerHTML;
                const denominator = this.root.querySelector('.denominator_value').innerHTML;
                const quotient = Math.floor(numerator / denominator);
                const answer = this.root.querySelector('remote-html-embed').shadowRoot
                    .querySelector('#st1_1_2_value3').value;
                if(parseInt(answer) !== quotient)
                    isComplete = false;
                if(isComplete) {
                    const index = this.root.querySelector(".view-wrap")?.dataset.index;
                    dispatchEvent(this.root, {type: "popup", target: parseInt(index) + 1});
                }else{
                    this.root.querySelector('remote-html-embed').shadowRoot.showActivityMessage();
                }
            };
        }
