...
export default class StepSecondActivity {
    ...
    SetDragAreaBlock(dragArea, index, type) {
        ...
        //SEO
        const quotient = Math.floor(this.main.value1/this.main.value2);
        if(this.dragAreaNumList[index] === quotient){
            this.CheckDrag(true);
        }
    ...
    CheckDrag(sendDispatch = false) {
        const quotient = Math.floor(this.main.value1 / this.main.value2);
        const remainder = this.main.value1 % this.main.value2;
        let isComplete = true;
        for (let i = 0; i < this.dragAreaNumList.length; i++) {
            if (this.dragAreaNumList[i] !== quotient) {
                isComplete = false;
                break;
            }
        }
        const remainderCount = this.left_cnt_container.innerText?parseInt(this.left_cnt_container.innerText):0;
        // 완료 되었음을 알려준다.
        if (isComplete && remainderCount === remainder) {
            if (sendDispatch) {
                const args = {
                ...
                this.main.shadow.dispatchEvent(event);
            }
            return true;
        }
        return false;
